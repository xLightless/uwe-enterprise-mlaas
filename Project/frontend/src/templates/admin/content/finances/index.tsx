...
import { AdminGetClaimsByStatus, FinanceClaimsToSettle, mapAdminGetClaimsByStatus, mapFinanceClaimsToSettle, TableRow } from "../../../../common/interfaces";
import Overlay from "../../../../components/overlay";
import { adminGetClaimsByStatus, adminBulkUpdateClaims } from "../../../../repositories/claims";
import { getApprovedClaimsToSettle } from "../../../../repositories/finance";

const Invoices: React.FC = () => {
    const [settledClaims, setSettledClaims] = React.useState<AdminGetClaimsByStatus | null>(null);
    const [approvedClaims, setApprovedClaims] = React.useState<FinanceClaimsToSettle | null>(null);
    ...
    async function fetchClaims() {
        ...
        const fetchApprovedClaims = await getApprovedClaimsToSettle();
        const mappedApprovedClaims = mapFinanceClaimsToSettle(fetchApprovedClaims as unknown as Record<string, unknown>);
        setApprovedClaims(mappedApprovedClaims as unknown as FinanceClaimsToSettle);
    ...
    async function onButtonSettle() {
        if (selectedClaims.length > 0) {
            await adminBulkUpdateClaims(selectedClaims, "settled")
            const response = await adminGetClaimsByStatus("settled");
            const mappedSettledClaims = mapAdminGetClaimsByStatus(response as unknown as Record<string, unknown>);
            setSettledClaims(mappedSettledClaims);
        }
    };
    ...
    return (
        <div className="w-full h-full grid grid-rows-[0.25fr_1fr]">
            <div className='w-full h-full grid grid-rows-[auto_1fr]'>
                <div>
                    <h1 className='font-bold text-2xl'>Review Finances</h1>
                    <p className='text-sm text-gray-500'>Settle any approved claims.</p>

                    {/* Statistics about pending claims */}
                    <div className='grid grid-cols-2 gap-4 mt-4'>
                        <div className='bg-white shadow-md rounded-lg p-4'>
                            <h2 className='text-lg font-semibold'>Approved Claims</h2>
                            <p className='text-2xl font-bold'>{JSON.stringify(approvedClaims?.claims.length)}</p>
                        </div>
                        <div className='bg-white shadow-md rounded-lg p-4'>
                            <h2 className='text-lg font-semibold'>Settled Claims</h2>
                            <p className='text-2xl font-bold'>{JSON.stringify(settledClaims?.claims.length) || "0"}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className='my-4 flex justify-center items-center w-full'>
                    ...
                    <div className='w-full grid grid-rows-1 md:flex md:flex-wrap md:items-center md:justify-between gap-2 bg-gray-200 rounded-lg shadow-md py-4 px-2'>
                        <div className='space-x-2'>
                            <button className='bg-green-500 text-white px-4 py-2 rounded-lg cursor-pointer hover:bg-green-400' onClick={() => onButtonSettle()}>Settle</button>
                        </div>
                            ...
                            <div className="bg-white rounded grid grid-rows-[auto_1fr_auto] p-4">
                                ...
                                <PaginatedTable
                                        ...
                                        tbody={selectedClaims.map((claimId) => {
                                            const claim = approvedClaims?.claims.find((claim) => claim.claimId === claimId);
                                            return {
                                                ...
                                                hasStripeAccount: JSON.stringify(claim?.hasStripeAccount) || "No Stripe Account",
                                                // stripeAccountReady: JSON.stringify(claim?.stripeAccountReady) || "No Stripe Account",
                                                // canLinkTestAccount: JSON.stringify(claim?.canLinkTestAccount) || "No Stripe Account",
