...
from .serializer import PermissionSerializer, RolePermissionSerializer
...
def create_permission(request):
    ...
    if serializer.is_valid():
        ...
        return Response({"message": "Permission created and attached to role successfully.", "data": serializer.data}, status=status.HTTP_201_CREATED)
...
def view_permissions_by_role(request, role_id):
    try:
        ...
        return Response({"data": grouped_permissions}, status=status.HTTP_200_OK)
    except Role.DoesNotExist:
        return Response({"error": "Role not found."}, status=status.HTTP_404_NOT_FOUND)
...
def update_permission(request, permission_id):
    ...
    if permission_name:
        permission.permission_name = permission_name.lower()  # Ensure it's stored in lowercase
        permission.save()
