...
LABEL_ENCODERS_PATH = os.path.join(os.path.dirname(__file__), 'label_encoders.pkl')
...
def preprocess_value(feature, value):
    if feature in ["Accident Date", "Claim Date"]:
        try:
            ...
            return (dt - datetime.datetime(1970, 1, 1)).total_seconds() / (24 * 3600)
        except ValueError:
            raise ValueError(f"Invalid date format for {feature}. Expected format: 'DD/MM/YYYY HH:MM:SS'")
    elif feature in ["Police Report Filed", "Witness Present", "Whiplash", "Exceptional_Circumstances", "Minor_Psychological_Injury"]:
        return 1 if value in ["Yes", True, "true", "yes"] else 0
    elif feature in ["SpecialHealthExpenses", "SpecialReduction", "SpecialOverage", "GeneralRest",
...
def generate(settlement_amount, lime_explanation):
    ...
    contents = [
        types.Content(
            role="user",
            parts=[types.Part.from_text(text=f"Settlement Amount: {settlement_amount}\nLIME Explanation: {lime_explanation}")],
        ),
    ]
    generate_content_config = types.GenerateContentConfig(
        response_mime_type="text/plain",
        system_instruction=[
            types.Part.from_text(text="""using the features and values given, the settlement amount given and the LIME explanations, give a reason why that settlement amount was chosen"""),
        ],
    )
...
def predict(request):
    try:
        ...
        transformed_data = [preprocess_value(feature, input_data.get(feature, None)) for feature in features]
